    ...
    public LogIn(){
        ...
        LoginKey.addActionListener((ActionEvent e) -> {
            ...
            if (!usernamestr.equalsIgnoreCase("") && !passwordstr.equalsIgnoreCase("")) {
                if(!new methods().isManager(usernamestr,passwordstr) && !new methods().isOperator(usernamestr,passwordstr)) {
                ...
                else if(new methods().isOperator(usernamestr,passwordstr)){
                    this.setVisible(false);
                    try {
                        Operator operator = new Operator(usernamestr,passwordstr);
                    ...
                    try {this.setVisible(false);
                        Manager manager = new Manager(usernamestr);
                    } catch (IOException e1) {
                        e1.printStackTrace();
                    }
                }
